...
checkboxInput(
  inputId = "filter_on",
  label = strong("Optional filter question"),
  value = 0
)
```
...
```{r filter-options}
get_question_choices <- function(question_name) {
  return(rep(question_name, 3))
}

renderUI({
  choices <- get_question_choices(input$filter_question_name)
  conditionalPanel(
    'input.filter_question_name != "none"',
    checkboxGroupInput(
